...
<script type="text/javascript">
    $(function(){
        ...
        $("#imageCode").blur(function(){
            checkLength("imageCode", "imageCodeTip", 4);
        });
        ...
        $("form").submit(function(){
            ...
            var r3 = checkLength("imageCode", "imageCodeTip", 4);
      ...
      <form action="<%=path%>/user" method="post">
          ...
          <p>
             <input  id="rememberMe" name="rememberMe" type="checkbox"   value="yes"   
                <c:if test="${(not empty param.email) && (not empty param.rememberMe) }">
                     checked="checked"
                </c:if>
               <c:if test="${(empty param.email) && (not empty cookie.rememberMe.value) }">
                     checked="checked"
                </c:if>
             />
